...
frequency of trigrams for english language.  
    ...
    rie(types_freq_an_en_3,types_distrib,qc_full,"en",3,rem_stopw = T  
      ,faceted = fct)
the calculation function is types_distrib() that takes as parameters;  
...
Analysis code is distributed in several files. The main file that contains the analysis and plot functions is  
https://github.com/enrico200165/Data_Science_JHU_10_Natural-Language-Processing_R/blob/master/ev_nlp_eda_lib.R  
note that it depends on (sources) other files providing service functions (rie() for instance) and global flags ().
