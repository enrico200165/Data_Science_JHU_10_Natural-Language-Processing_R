...
setwd("/media/enrico/usbdata/data/pers_dev/sources/DS_capstone/Data_Science_JHU_10_Natural-Language-Processing_R/")


myCorpus <- corpus(readtext("./data/inaugural/*.txt"))
myCorpus <- corpus(readtext("./data/sotu/*.txt"))

mytf <- readtext("./data/inaugural/*.txt", docvarsfrom = "filenames", dvsep = "-", 
...
sotuCorpus <- corpus(readtext("./data/sotu/*.txt", encoding = "UTF-8-BOM"))
...
tf1 <- readtext("./data/inaugTexts.csv", text_field = "inaugSpeech")
myCorpus <- corpus(tf1)
tf2 <- readtext("./data/text_example.csv", text_field = "Title")
myCorpus <- corpus(tf2)
head(docvars(tf2))
